...
MOD_USER	VARCHAR(10) NOT NULL ,
MOD_DATE	DATE DEFAULT     SYSDATE ,
CONSTRAINT  PK_TB_CM_H_PR_BAT_APTMM PRIMARY KEY (BAT_YM, BJD_SGG_CODE , BJD_UMD_CODE , JIBUN, KAPT_NAME, AREA_UNIT  , YYYY, MM, DD, SEQ)
);
...
*-------------------------
*/
drop table TB_CM_H_PR_BAT_APTJS ;
 CREATE TABLE TB_CM_H_PR_BAT_APTJS 
(
BAT_YM              VARCHAR(6  ) NOT NULL,
AREA_CODE           VARCHAR(5  ) NOT NULL,
BJD_NAME            VARCHAR(40 ) NOT NULL,
...
SEQ                 NUMBER(10)   NOT NULL,
ST_YYYY             VARCHAR(4  ) NOT NULL,
DEPOSIT_AMT         VARCHAR(40 ) NOT NULL,
RENT_AMT            VARCHAR(40 ) NOT NULL,
PRE_DEPOSIT_AMT     VARCHAR(40 ) NOT NULL,
PRE_RENT_AMT        VARCHAR(40 ) NOT NULL,
CONTRACT_GBN        VARCHAR(20 ) NOT NULL,
CONTRACT_PER        VARCHAR(20 ) NOT NULL,
RE_NEW_USE          VARCHAR(10 ) NOT NULL,
FLOOR               VARCHAR(4  ) NOT NULL,
MOD_USER	        VARCHAR(10) NOT NULL ,
MOD_DATE	        DATE DEFAULT     SYSDATE ,
CONSTRAINT  TB_CM_H_PR_BAT_APTJS PRIMARY KEY (BAT_YM, AREA_CODE, BJD_NAME, JIBUN, KAPT_NAME, YYYY, MM, DD, SEQ) 
);
COMMENT ON TABLE TB_CM_H_PR_BAT_APTJS                     IS '공통_H_시세_배치_아파트_전세';
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.BAT_YM             IS '조회년월'					 ;
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.DEPOSIT_AMT        IS '보증금액(만원)'     ;
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.RENT_AMT           IS '월세금액'     ;
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.PRE_DEPOSIT_AMT    IS '종전보증금액(만원)'     ;
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.PRE_RENT_AMT       IS '종전월세금액'     ;
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.CONTRACT_GBN       IS '계약구분'          ;
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.CONTRACT_PER       IS '계약기간'     ;
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.RE_NEW_USE         IS '갱신요규권사용'     ;
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.ST_YYYY            IS '건축년도'          ;
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.YYYY               IS '년'              ;
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.MM                 IS '월'              ;
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.DD                 IS '일'              ;   
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.BJD_NAME           IS '법정동'           ;
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.KAPT_NAME          IS '아파트'           ;
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.SEQ                IS '시퀀스'              ;
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.AREA_UNIT          IS '전용면적'          ;
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.JIBUN              IS '지번'             ;
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.AREA_CODE          IS '지역코드'          ;
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.FLOOR              IS '층'              ;
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.MOD_USER		      IS '수정자'           ;
COMMENT ON COLUMN TB_CM_H_PR_BAT_APTJS.MOD_DATE			  IS '수정일시'          ;
